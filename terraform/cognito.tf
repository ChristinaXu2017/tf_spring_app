...
# Pool and client
# 
resource "aws_cognito_user_pool" "VaccineUserPool" {
  name = "vaccine-users"

  admin_create_user_config {
    allow_admin_create_user_only = true
    invite_message_template {
      email_subject = "Vaccine project registration"
      email_message = "Welcome to Vaccine project, please use username: {username} and password: {####} to log in."
      sms_message   = "Welcome to Vaccine project, please use username: {username} and password: {####} to log in."
    }
  }
...
resource "aws_cognito_user_pool_client" "VaccineUserPool_client" {
  name = "vaccine-users-client"

  user_pool_id = aws_cognito_user_pool.VaccineUserPool.id
...
data "aws_iam_policy_document" "admin-group-role-policy" {
  statement {
    ...
    resources = [
      aws_cognito_user_pool.VaccineUserPool.arn
    ]
  }
}
...
resource "aws_cognito_user" "guest" {
  user_pool_id = aws_cognito_user_pool.VaccineUserPool.id
  username     = var.vaccine-guest-username
  password     = var.vaccine-guest-password

  attributes = {
    terraform      = true
    email          = var.vaccine-guest-username
    email_verified = true
    given_name     = "Guest"
    family_name    = "Guest"
  }
...
resource "aws_cognito_user" "admin" {
  user_pool_id = aws_cognito_user_pool.VaccineUserPool.id
  username     = var.vaccine-admin-username
  password     = var.vaccine-admin-password

  attributes = {
    terraform      = true
    given_name     = "Admin"
    family_name    = "Admin"
    email          = var.vaccine-admin-username
    email_verified = true
  }
}
...
# authorizers
# 
resource "aws_api_gateway_authorizer" "VaccineUserPool-authorizer" {
  name          = "UserPoolAuthorizer-vaccine"
